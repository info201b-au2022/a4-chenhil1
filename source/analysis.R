...
# Get dataframe of female prisoner proportions
map_data <- function() {
  map <- poc %>%
    mutate(female_prop = female_prison_pop / female_pop_15to64) %>%
    drop_na() %>%
    select(year, state, female_prop) %>%
    group_by(state) %>%
    summarise(mean(female_prop)) %>%
    rename("avg_female_prop" = "mean(female_prop)")

  return(map)
}

# Plot and format map of female prisoner proportions
plot_map_data <- function() {
  p <- plot_usmap(regions = "state", data = map_data(), values = "avg_female_prop", color = "black") +
    ...
    labs(
      x = "Longitude",
      y = "Latitude",
      title = "Average Proportion of Female Prison Population
      Imprisoned by State (1970-2018)",
      caption = "Geographic distribution of proportions across the U.S. 
      (greyed-out states had near-zero proportions or no data available)."
    ) +
    theme_minimal()
